...
cd "C:\Users\richard\OneDrive - Universidad Nacional Mayor de San Marcos\Curso de STATA\Construcción de indicadores\00_Pobreza monetaria"
...
**************************
*Para renombrar caracteres que no reconoce stata usamos el comodin "*" en lugar de la variable
rename a*o anio
*Departamento
...
*Calculamos el factor de ponderacion a nivel de la poblacion, el que nos da stata en SUMARIAS  se encuentra a nivel del hogar y por eso se hace la multiplicacion del factor por el n umero de miembros del hogar
gen facpob=factor*mieperho

*Contrastamos gpcm con la linea de pobreza alimentaria (linpe) "ayuda a distinguir los pobres extremos del resto de la población" y pobreza total (linea) "identifica a los pobres de los no pobres"
...
tab pobre3
*Generamos la variable pobreza pero distinguiendo solo dos categorías "POBRE y NO POBRE"
...
sepov   gpcm [w=facpob], p(linea)
outreg using pobreza.doc, replace stat(b se ci) nosubstat bdec(4) varlabels ///
 ctitle("Indicador","","Error Estandar","Intervalo de confianza al 95%") ///
title("Pobreza Monetaria - 2019") note("Fuente: ENAHO 2018")

sepov   gpcm [w=facpob], p(linea) by(area)
outreg using pobreza.doc, addtable stat(b se ci) nosubstat bdec(4) varlabels ///
 ctitle("Indicador","Ambito","","Error Estandar","Intervalo de confianza al 95%") ///
title("Pobreza Monetaria - 2019") note("Fuente: ENAHO 2018")

*Estimando el porcentaje de la población POBRE Y NO POBREZA
svy: prop pobre_2
outreg using pobreza.doc, addtable stat(b se ci) nosubstat bdec(4) varlabels ///
 ctitle("","Pobreza","Error Estandar","Intervalo de confianza al 95%") ///
title("Pobreza Monetaria - 2018") note("Fuente: ENAHO 2018")

svy: prop pobre_2, over(area)
outreg using pobreza.doc, addtable stat(b se ci) nosubstat bdec(3) varlabels ///
 ctitle("","Ambito","Pobreza","Error Estandar","Intervalo de confianza al 95%") ///
title("Pobreza Monetaria por ambito - 2018") note("Fuente: ENAHO 2018")

svy: prop pobre_2, over(regnat)
outreg using pobreza.doc, addtable stat(b se ci) nosubstat bdec(3) varlabels ///
 ctitle("","Region natural","Pobreza","Error Estandar","Intervalo de confianza al 95%") ///
title("Pobreza Monetaria por region natural - 2018") note("Fuente: ENAHO 2018")


svy: prop pobre_2, over(dpto)
outreg using pobreza.doc, addtable stat(b se ci) nosubstat varlabels bdec(3)  ///
 ctitle("","Departamento","Pobreza","Error Estandar","Intervalo de confianza al 95%") ///
title("Pobreza Monetaria por departamento - 2018") note("Fuente: ENAHO 2018")
